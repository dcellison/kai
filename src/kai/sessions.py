...
async def init_db(db_path: Path) -> None:
    ...
    # ── Schema migration from single-user to multi-user ──────────────
    await _migrate_to_multi_user()


async def _migrate_to_multi_user() -> None:
    """
    Migrate existing single-user database to multi-user schema.

    Detects old schema by checking for user_id column presence and
    recreates tables with the new schema, copying data with
    user_id = chat_id (which is correct for single-user: user_id == chat_id
    in private chats).
    """
    # Check if sessions table has old schema (chat_id PK, no user_id column)
    cursor = await _get_db().execute("PRAGMA table_info(sessions)")
    columns = {row[1] for row in await cursor.fetchall()}

    if "chat_id" in columns and "user_id" not in columns:
        # Old sessions schema: chat_id PK → new: user_id PK + chat_id
        await _get_db().execute("""
        ...
        """)
        await _get_db().execute("""
            INSERT INTO sessions_new (user_id, chat_id, session_id, model, created_at, last_used_at, total_cost_usd)
            SELECT chat_id, chat_id, session_id, model, created_at, last_used_at, total_cost_usd
            FROM sessions
        """)
        await _get_db().execute("DROP TABLE sessions")
        await _get_db().execute("ALTER TABLE sessions_new RENAME TO sessions")

    # Check if jobs table has old schema (no user_id column)
    ...
    if "user_id" not in job_columns:
        ...
        """)
        # Handle case where notify_on_check may not exist in old schema
        if "notify_on_check" in job_columns:
            await _get_db().execute("""
                INSERT INTO jobs_new (id, user_id, chat_id, name, job_type, prompt, schedule_type, schedule_data, created_at, active, auto_remove, notify_on_check)
                SELECT id, chat_id, chat_id, name, job_type, prompt, schedule_type, schedule_data, created_at, active, auto_remove, notify_on_check
                FROM jobs
            """)
        else:
            await _get_db().execute("""
                INSERT INTO jobs_new (id, user_id, chat_id, name, job_type, prompt, schedule_type, schedule_data, created_at, active, auto_remove)
                SELECT id, chat_id, chat_id, name, job_type, prompt, schedule_type, schedule_data, created_at, active, auto_remove
                FROM jobs
            """)
        await _get_db().execute("DROP TABLE jobs")
        await _get_db().execute("ALTER TABLE jobs_new RENAME TO jobs")

    # Check if settings table has old schema (key PK, no user_id)
    cursor = await _get_db().execute("PRAGMA table_info(settings)")
    ...
    if "user_id" not in settings_columns:
        ...
        """)
        # Migrate old settings: global keys (no colon) get user_id=0 as placeholder;
        # namespaced keys like "voice_mode:12345" get the chat_id extracted.
        rows = await _get_db().execute("SELECT key, value FROM settings")
        for row in await rows.fetchall():
            key, value = row["key"], row["value"]
            if ":" in key:
                # Extract user_id from namespaced key like "voice_mode:12345"
                parts = key.rsplit(":", 1)
                try:
                    uid = int(parts[1])
                    clean_key = parts[0]
                    await _get_db().execute(
                        "INSERT OR REPLACE INTO settings_new (user_id, key, value) VALUES (?, ?, ?)",
                        (uid, clean_key, value),
                    )
                except ValueError:
                    # Not a numeric suffix; treat as global with user_id=0
                    await _get_db().execute(
                        "INSERT OR REPLACE INTO settings_new (user_id, key, value) VALUES (?, ?, ?)",
                        (0, key, value),
                    )
            else:
                # Global key like "workspace" - store with user_id=0
                await _get_db().execute(
                    "INSERT OR REPLACE INTO settings_new (user_id, key, value) VALUES (?, ?, ?)",
                    (0, key, value),
                )
        await _get_db().execute("DROP TABLE settings")
        await _get_db().execute("ALTER TABLE settings_new RENAME TO settings")

    # Check if workspace_history has old schema (path PK, no user_id)
    cursor = await _get_db().execute("PRAGMA table_info(workspace_history)")
    wh_columns = {row[1] for row in await cursor.fetchall()}

    if "user_id" not in wh_columns:
        await _get_db().execute("""
        ...
        """)
        # Migrate with user_id=0 placeholder (will be associated with actual users on use)
        await _get_db().execute("""
            INSERT INTO workspace_history_new (user_id, path, last_used_at)
            SELECT 0, path, last_used_at FROM workspace_history
        """)
