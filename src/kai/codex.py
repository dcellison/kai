...
class CodexBackend(AgentBackend):
    ...
    async def _ensure_started(self) -> None:
        ...
        thread_params: dict = {
            ...
            # `danger-full-access` matches the claude backend's
            # posture: claude --print runs unsandboxed with whatever
            # permissions the bot's os_user has. The `workspace-write`
            # alternative would disable network access (codex's
            # default for workspace-write profiles), breaking `gh`,
            # `curl`, `pip`, and anything else the bot needs to
            # reach outside the local filesystem. The bot already
            # runs under a per-user sudo wrap with that user's full
            # file authority; constraining codex tighter than the
            # surrounding process makes no security sense.
