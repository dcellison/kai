...
| Canonical timeline query and client synchronization | A production-unused canonical query, concrete channel policy, and versioned read-only HTTP contract now define authorized channel reads and snapshot-stable pagination; durable human-client sessions, production route registration, and a resumable event stream do not yet exist | Partial |
...
4. **Authenticated read-only diagnostic surface:** the production-unregistered HTTP contract now exposes canonical, snapshot-stable timeline pages through an injected human-client authenticator. It reuses principal/channel authorization, reveals no cross-channel data, accepts no transport identity, and registers no write methods. Production registration remains blocked on durable, revocable human-client sessions with device tracking. Backend-process credentials are a separate trust domain and must not be reused as human sessions.
...
The next code PR is the remaining authentication half of item 4: a durable, revocable human-client session foundation with device tracking. It must remain separate from backend-process credentials and must stay production-unused until its lifecycle and authorization contracts are pinned. Only then may a later PR register the read-only timeline route in the local production server.
