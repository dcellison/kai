...
### 35.1 Next bounded milestone

After merge and install, qualify one disposable client against the deployed
event stream: hand off from a timeline `through_position`, observe one new
Telegram-authored canonical message, restart Kai, resume from its exact SSE
event ID without replay, observe one later message, revoke the device, and
confirm the stream can no longer reconnect. Then begin the first minimal
Workshop client shell against the proven enrollment, timeline, and event-stream
contracts; command submission remains a separate authorization milestone.
