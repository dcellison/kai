...
Protected canonical private text now completes this scoped transcript cutover.
It performs no JSONL reads or writes. Fresh runtime context comes from the
bounded Workshop timeline; semantic-memory episode windows use prior canonical
human/agent pairs; and new memory source views verify the exact canonical
principal, channel, run, source message, and result message in SQLite.

For history outside the bounded prompt window, each protected execution
refreshes a derived ``canonical-transcript.ndjson`` snapshot in the channel's
existing protected history directory. The snapshot contains canonical IDs and
message facts only, is atomically replaced, inherits the channel's named-user
read policy, and is ignored by compatibility ``*.jsonl`` readers. The operator
can also emit the same format explicitly with ``kai workshop transcript
export --channel-id ...``. SQLite remains authoritative; the snapshot is a
search/export projection and can always be rebuilt.

This is still not global JSONL retirement. Media, commands, groups, schedules,
and integrations remain compatibility routes until Milestone 5 and therefore
retain their JSONL reads/writes and archived provenance. Their migration—not
more private-text transcript work—is the removal gate for the remaining JSONL
implementation. ``install status`` reports this distinction explicitly.
