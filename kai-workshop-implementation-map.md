...
| Canonical timeline query and client synchronization | A production-unused canonical query, concrete channel policy, versioned read-only HTTP contract, and durable hashed-token human-session foundation now define authorized channel reads and snapshot-stable pagination; secure client enrollment, production route registration, and a resumable event stream do not yet exist | Partial |
...
The next code PR should define the secure client-enrollment boundary for item 4. It must bind the initial device and session to a principal established by the server or operator, never by an untrusted client identity claim, and must preserve the separation from backend-process credentials. Session/device rows are mutable security state rather than replayed collaboration projections; later append-only audit events may record their lifecycle but must never be capable of reactivating a credential. Production route registration remains a separate, explicit decision after enrollment is proven.
