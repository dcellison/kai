...
| Direct Telegram delivery from handlers, schedules, and webhooks | Durable delivery outbox and Telegram delivery adapter | Durable fragment progress and the unavoidable send/commit ambiguity have an explicit policy; retry, crash recovery, ordering, private-chat and notification-group delivery tests pass; live delivery is verified | Register the outbox worker only in an explicit cutover; remove direct Bot API sends from domain paths and delete delivery fallback flags after installed verification | Active (production-unused durable request/lease/attempt/retry/recovery foundation plus unregistered single-message Telegram adapter/worker; no enqueue, startup registration, or authority change) |
...
| Durable outbound delivery | A production-unused foundation atomically records `delivery.requested` with pending work, uses exclusive expiring leases and immutable attempt records, applies bounded deterministic retry policy, rejects stale completion, and recovers expired work after restart. An unregistered Telegram text adapter/worker claims only matching transport/mode work, targets canonical private/group bindings, preserves Kai's Markdown fallback, sanitizes provider failures, and leaves unexpected errors or cancellation leased for recovery. Tests cover concurrent claims, request rollback, retry exhaustion, restart recovery, target isolation, notification groups, and projection rebuild preservation. It deliberately accepts only one bounded Telegram message: durable multi-fragment progress and the external-send/database-commit ambiguity remain unresolved activation blockers. | Partial (adapter/worker contract implemented but unregistered; no enqueue, live recovery evidence, fragment model, outcome-event integration, or authority change) |
...
6. **Durable delivery outbox:** the production-unused foundation defines atomic `delivery.requested` work, immutable attempts, exclusive expiring claims, bounded retry/failure policy, stale-lease rejection, and crash recovery. Its unregistered Telegram adapter/worker handles one bounded text send to canonical private or notification-group bindings, claims no other transport/mode, and leaves unexpected interruption recoverable. It neither registers at startup nor enqueues or replaces a direct Telegram send. Durable fragment checkpoints and explicit handling of the external-send/database-commit ambiguity are required before activation.
...
Successful installed voice ingress, service restart, and clean canonical-to-JSONL parity are now recorded, including a clean post-outbox install at 32 canonical/projected messages. After the production-unused Telegram adapter/worker merges, the next bounded delivery milestone should design and test durable Telegram fragment progress and outcome-event integration, explicitly documenting the remaining external-send/database-commit ambiguity, before any startup registration or live enqueue path is considered. Operator-facing grant issuance and production route registration remain separate, explicit decisions requiring rate limits, secure transport assumptions, and installed restart/revocation evidence.
