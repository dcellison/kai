...
The remaining gate is installed evidence: deploy the bootstrap, prepare the configured notification group, run only the returned delivery, confirm the recognizable message reaches that group, verify its terminal binding-aware status, and confirm that ordinary GitHub notifications still reach the same group through the unchanged legacy route. Only then should lifecycle ownership and another authority review proceed.
