...
|---|---|---|---|---|
| Authenticated plain-text canonical shadow writes beside current history | Workshop event store and message projection | Deterministic replay, duplicate-ingress tests, restart tests, media-ingress coverage, and sustained parity with current history | Remove the `workshop_inbound_recorder` bot-data adapter and fail-open handler branch; make canonical command/event transactions the sole message write path | Active (inbound text) |
