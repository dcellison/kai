...
6. **Durable delivery outbox:** the production-unused foundation defines atomic `delivery.requested` work, immutable attempts, exclusive expiring claims, bounded retry/failure policy, stale-lease rejection, and crash recovery. Terminal settlement now atomically records a binding-aware version-2 success or failure fact, while legacy version-1 shadow observations remain replayable. Its unregistered Telegram adapter/worker handles durably planned, ordered text fragments for canonical private or notification-group bindings and claims no other transport/mode. Confirmed fragments resume without resend. The external-send/database-commit ambiguity is fail-closed: an in-flight fragment without confirmation becomes terminally uncertain and requires reconciliation rather than automatic retry. It neither registers at startup nor enqueues or replaces a direct Telegram send. A cross-delivery ordering contract and installed live recovery evidence remain required before activation.
...
Successful installed voice ingress, service restart, and clean canonical-to-JSONL parity are now recorded, including a clean post-outbox install at 32 canonical/projected messages. Binding-aware `delivery.succeeded` and `delivery.failed` facts are now persisted atomically with terminal outbox settlement, without startup registration or live enqueue. The next bounded delivery milestone is an explicit ordering contract across multiple deliveries to one binding; installed live recovery remains a later gate. Operator-facing grant issuance and production route registration remain separate, explicit decisions requiring rate limits, secure transport assumptions, and installed restart/revocation evidence.
