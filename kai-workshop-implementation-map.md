...
| Media ingress and artifact provenance | Canonical artifact identity, additive metadata storage, deterministic projection replay, and fail-open Telegram photo/document adapters now record content hash, size, normalized MIME type, owning message/channel/principal, non-retrievable transport unique ID, bounded original filename, and local storage provenance. Voice ingress does not yet shadow into it. | Partial |
...
5. **Media and artifact shadow:** the canonical artifact foundation records content identity and bounded local-storage provenance only for an existing human-authored canonical message. Successful Telegram photos and documents now shadow through the message and artifact recorders, mark only confirmed canonical message writes for legacy parity, and fail open without changing current files, prompts, agent invocation, or Telegram delivery. Document metadata uses a normalized valid transport MIME type, a deterministic fallback, and a bounded cross-platform basename. Wire voice ingress as the next separate adapter-specific step.
...
The next code PR should apply the proven fail-open adapter contract to successful Telegram voice ingress while leaving transcription, prompts, agent invocation, JSONL history, and Telegram responses authoritative and unchanged. Operator-facing grant issuance and production route registration remain separate, explicit decisions requiring rate limits, secure transport assumptions, and installed restart/revocation evidence.
