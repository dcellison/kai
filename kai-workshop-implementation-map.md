...
| Schedule firing directly into the pool or Telegram | Durable Workshop run creation | Scheduled definitions and executions survive restart and expose run/attempt state without duplicate work | Remove schedule-specific execution path; retain schedules only as authenticated run triggers | Retired. The core-owned scheduler creates canonical reminders or runs and requests delivery through the outbox; legacy job definitions are a frozen archive. |
| GitHub and generic webhook paths that route directly to Telegram or the pool | Canonical integration commands/events plus delivery/run services | Existing GitHub group notifications, generic callers, deduplication, and secret separation pass end-to-end tests | Remove direct routing while retaining verified webhook adapters and supported external contracts | Retired. GitHub notification, review, triage, and generic webhook routing are canonical; configured Telegram delivery is an outbox effect. |
| Per-chat settings, files, memory references, and project selection, including rollback dual writes from canonical execution state to `settings`, `workspace_history`, and `allowed_workspaces` | Channel-agent execution/workspace settings, principal workspace history/grants, principal-owned storage, and artifact metadata | Existing per-user isolation, restart restoration, workspace overrides/access, context assembly, memory provenance, and file delivery remain equivalent; installed diagnostics show every protected profile migrated with no missing/stale owner; legacy-row tampering cannot affect protected reads | After installed qualification, stop rollback writes to the three numeric stores and remove their protected-profile rows/read compatibility; retire recorded compatibility `storage_path` values when an authoritative artifact store replaces local per-chat files | Active (schema v21 makes model/timeout/active-workspace settings, workspace overrides/history, and mutable grants canonical for protected profiles; numeric stores are read-disabled there and receive temporary same-transaction rollback writes; principal-named files, home, preferences, and memory directories are active, while numeric upload archives remain transitional) |
| Protected semantic-memory runtime-ID alias and schema-v22 migration receipts | Canonical principal namespace with explicit principal/channel/agent/runtime-profile/run/message provenance | Idempotent partial-migration tests, cross-principal isolation, stable memory IDs/content, conflicting-owner denial, canonical-only protected reads, clean installed authority diagnostics, restart recall, and new-memory provenance inspection pass | After every production caller and operator command supplies canonical authority directly, remove the runtime-ID resolver and receipt-backed offline alias; retain the receipts as migration history until the rollback/archive window closes | Active (schema v22 re-keys existing Mem0 rows in place and rejects unowned protected reads; new Workshop private-text memories carry canonical run and message provenance; JSONL source locators remain supplemental archive provenance pending transcript retirement. Rollback across v22 must restore Qdrant and SQLite from the same pre-migration backup; numeric rows written by older code after a retained v22 receipt are not automatically re-imported.) |
| Numeric scheduled-job ownership, per-user GitHub subscription policy, and per-chat compatibility locks | Canonical principal/channel/agent/runtime-profile job ownership, principal-owned GitHub subscription policy, and channel-agent execution lanes | Existing schedules and GitHub notification subscriptions survive restart; protected reads ignore numeric compatibility tampering; cross-owner job mutation is denied; lane cancellation/serialization remains isolated; installed diagnostics report no missing, stale, unowned, or conflicting state | After the shared scheduler and integration services consume canonical identities directly, remove `jobs.chat_id`, the rollback GitHub settings writes, and runtime-ID lane translation; retain migration receipts during the rollback window | Active (schema v23 backfills immutable canonical job owners and principal GitHub subscription policy; verified receipts safely adopt jobs created during a supported rollback window; protected job/policy reads are canonical; root-managed repository baselines and non-user toggle defaults resynchronize at startup while interactive overrides persist; wildcard routing follows the human's current Workshop role; writes retain rollback compatibility; Telegram schedule execution still carries the numeric runtime alias privately until the shared-service cutover) |
...
## 47. Telegram-independence transition-tooling retirement

**Implementation date:** 2026-08-25

The final source-simplification package removes transition mechanisms whose
installed removal gates have passed. The per-chat `.responding` and
`.responding_to` crash flags are obsolete because durable canonical runs,
attempts, terminal transactions, and recovery own interruption state. The
combined canonical-shadow/JSONL parity comparator is obsolete because
canonical replay integrity and legacy archive classification now have separate
authorities. The manually claimed Telegram delivery-qualification command and
delivery-authority rollback command are obsolete because supervised production
workers and canonical/outbox publication have replaced their qualification and
cutover roles.

This retirement deliberately does not delete historical data. Existing JSONL
files remain an owner-gated, non-authoritative archive for older memory
provenance. Historical qualification-purpose delivery rows, delivery-authority
epochs, migration receipts, numeric archives, and canonical job archives remain
readable evidence. Delivery epochs continue as the production worker's fencing
contract; they are not an operator-selectable authority switch.

`make install-status` reports the executable transition tooling as retired and
explicitly distinguishes retained archives. Static regression tests prevent
the removed shadow recorders, crash flags, parity comparator, qualification
