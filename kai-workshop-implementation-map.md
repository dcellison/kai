...
| Media ingress and artifact provenance | Canonical artifact identity, additive metadata storage, deterministic projection replay, and a fail-open Telegram photo adapter now record content hash, size, MIME type, owning message/channel/principal, non-retrievable transport unique ID, and bounded local storage provenance. Document and voice handlers do not yet shadow into it. | Partial |
...
5. **Media and artifact shadow:** the canonical artifact foundation records content identity and bounded local-storage provenance only for an existing human-authored canonical message. Successful Telegram photos now shadow through the message and artifact recorders, mark only confirmed canonical message writes for legacy parity, and fail open without changing current files, prompts, agent invocation, or Telegram delivery. Wire document and voice ingress in separate adapter-specific steps.
...
The next code PR should apply the proven fail-open adapter contract to successful Telegram document ingress while leaving file saving, prompts, agent invocation, JSONL history, and Telegram responses authoritative and unchanged. Voice wiring should follow separately after document MIME and filename provenance are pinned. Operator-facing grant issuance and production route registration remain separate, explicit decisions requiring rate limits, secure transport assumptions, and installed restart/revocation evidence.
