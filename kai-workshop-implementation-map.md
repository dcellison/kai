...
| Canonical timeline query and client synchronization | The live loopback HTTP server now exposes only rate-limited grant redemption and authenticated, authorized, snapshot-stable timeline reads. Operator-only commands issue a short-lived grant for an existing canonical Telegram human and revoke either the unredeemed grant or a device and all its sessions. Tokens are shown once and stored only as hashes; the first installed enrollment/restart/revocation qualification and a resumable event stream remain outstanding. | Partial |
