...
| Installed-system parity across fresh messages and a service restart | Clean installed smoke observations exist, but the diagnostic has not yet accumulated reviewed, sustained clean observations from the deployed installation | Not yet proven |
...
| Durable outbound delivery | A production-unused foundation atomically records `delivery.requested` with pending work, uses exclusive expiring leases and immutable attempt records, applies bounded deterministic retry policy, rejects stale completion, and recovers expired work after restart. Tests cover concurrent claims, request rollback, retry exhaustion, restart recovery, and preservation across canonical projection rebuilds. No production worker, transport adapter, or live enqueue path uses it, so direct Telegram delivery remains authoritative. | Partial (durability contracts implemented; production integration and live recovery evidence deliberately absent) |
...
6. **Durable delivery outbox:** the production-unused foundation defines atomic `delivery.requested` work, immutable attempts, exclusive expiring claims, bounded retry/failure policy, stale-lease rejection, and crash recovery. It neither registers a worker nor enqueues or replaces a direct Telegram send.
...
Successful installed voice ingress, service restart, and clean canonical-to-JSONL parity are now recorded. After the production-unused outbox foundation merges, the next bounded code milestone should define and test a production-unused Telegram delivery adapter/worker against that contract, including notification-group targeting, without registering the worker or moving delivery authority. Operator-facing grant issuance and production route registration remain separate, explicit decisions requiring rate limits, secure transport assumptions, and installed restart/revocation evidence.
