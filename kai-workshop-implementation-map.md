...
| Media ingress and artifact provenance | Canonical artifact identity, additive metadata storage, deterministic projection replay, and a production-unused recorder now define content hash, size, MIME type, owning message/channel/principal, non-retrievable transport unique ID, and bounded local storage provenance. Photo, document, and voice handlers do not yet shadow into it. | Partial |
...
5. **Media and artifact shadow:** the production-unused canonical artifact foundation now records content identity and bounded local-storage provenance only for an existing human-authored canonical message. Wire successful photo, document, and voice ingress into this recorder in small adapter-specific steps while preserving current files, prompts, and Telegram behavior.
...
The next code PR should shadow successful Telegram photo ingress into the canonical message and artifact recorders, fail open if shadowing fails, and leave file saving, prompts, agent invocation, JSONL history, and Telegram responses authoritative and unchanged. Document and voice wiring should follow only after the photo path proves the adapter contract. Operator-facing grant issuance and production route registration remain separate, explicit decisions requiring rate limits, secure transport assumptions, and installed restart/revocation evidence.
