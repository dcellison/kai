...
The client-state isolation required by [#835](https://github.com/dcellison/kai/issues/835) now separates mutable device, session, grant, and revocation rows from projection-owned principal deletion. Schema migration preserves existing security rows and explicit replay/restart tests prove that active and revoked state survives canonical projection rebuilds. Production registration still requires rate limits, secure transport assumptions, and installed enrollment/restart/revocation evidence.
