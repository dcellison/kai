...
The next operational step should install and exercise successful voice ingress, restart the service, and confirm canonical-to-JSONL parity remains clean. After that evidence, the next code milestone should define the production-unused durable delivery outbox foundation without replacing any direct Telegram send. Operator-facing grant issuance and production route registration remain separate, explicit decisions requiring rate limits, secure transport assumptions, and installed restart/revocation evidence.
