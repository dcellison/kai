...
class TestSendFile:
    ...
    async def test_authenticated_principal_can_send_from_canonical_outbox(self, isolated_send_file_roots):
        request, _workspace, data_dir = isolated_send_file_roots
        namespace = request.app[WORKSHOP_PRINCIPAL_STORAGE_KEY].for_runtime_profile(profile_id(123))
        staged = namespace.outbox_directory(data_dir) / "generated-report.md"
        staged.parent.mkdir(parents=True)
        staged.write_text("generated by the agent")
        request.json = AsyncMock(return_value={"path": str(staged)})
        ...
        publish.assert_awaited_once()
        assert publish.await_args.kwargs["path"] == staged
