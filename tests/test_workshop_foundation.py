...
class TestEventEnvelope:
    def test_event_vocabulary_includes_canonical_collaboration_and_run_lifecycle(self):
        assert {event_type.value for event_type in WorkshopEventType} == {
            ...
            "agent_definition.collaboration_policy_set",
            "agent_definition.archived",
            "agent_definition.authority_assigned",
            "principal_agent.enabled",
            "principal_agent.disabled",
            "principal_agent.runtime_changed",
            "principal_agent.conversation_started",
            "principal_direct_message.archived",
            "principal_direct_message.restored",
            "channel.agent_attached",
            "channel.agent_detached",
            ...
            "channel_read_position.advanced",
            "thread.followed",
            "thread.unfollowed",
            ...
            "run.failed",
            "run.cancelled",
            "agent_delegation.requested",
            "agent_delegation.started",
            "agent_delegation.completed",
            "agent_delegation.failed",
            "agent_delegation.cancelled",
            "run_attempt.granted",
            "run_attempt.started",
...
class TestWorkshopSchema:
    async def test_initial_schema_contains_domain_event_and_projection_records(self, workshop_store):
        expected = {
            ...
            "principal_appearance_preferences",
        ...
        assert await workshop_store.schema_version() == 81
        ...
        async with workshop_store.connection.execute("PRAGMA table_info(messages)") as cursor:
            message_columns = {str(row[1]) for row in await cursor.fetchall()}
        assert "thread_root_id" in message_columns
        async with workshop_store.connection.execute("PRAGMA table_info(channel_agents)") as cursor:
            channel_agent_columns = {str(row[1]) for row in await cursor.fetchall()}
        assert {
            "sponsor_principal_id",
            "sponsored_runtime_profile_id",
            "attached_event_position",
            "detached_at",
            "detached_event_position",
        } <= channel_agent_columns
    ...
    async def test_schema_migration_is_idempotent(self, tmp_path: Path):
        ...
        assert await second.schema_version() == 81
    ...
    async def test_expanded_reaction_migration_preserves_existing_reactions(self, tmp_path: Path, monkeypatch):
        ...
        try:
            assert await upgraded.schema_version() == 81
